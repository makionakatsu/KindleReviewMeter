...
export class BookInfoForm extends BaseComponent {
    constructor(container, context, options = {}) {
        ...
        Object.defineProperty(this, "bookModel", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "fetchStatus", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: { type: 'idle', message: '', timestamp: 0 }
        });
        Object.defineProperty(this, "saveInProgress", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "saveOperationId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        ...
        // パフォーマンス最適化
        Object.defineProperty(this, "updatePreviewDebounced", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    loadExistingData() {
        try {
            const savedData = this.context.storage.get('amazonReviewTracker');
            if (savedData) {
                // データ整合性検証
     ...
     * BookDataの構造を検証
     */
    validateBookDataStructure(data) {
    ...
    `;
        // DOM要素への参照を取得
        this.elements = {
            form: this.select('#settingsForm') ?? undefined,
            urlInput: this.select('#bookUrl') ?? undefined,
            fetchButton: this.select('#fetchAllBtn') ?? undefined,
            targetInput: this.select('#targetReviews') ?? undefined,
            stretchInput: this.select('#stretchReviews') ?? undefined,
            statusDisplay: this.select('#fetchAllStatus') ?? undefined,
            previewSection: this.select('.preview-section') ?? undefined,
            editAuthorButton: this.select('#editAuthorBtn') ?? undefined,
        };
    }
    /**
     * プレビューセクションのHTML
    ...
    populateForm() {
        ...
        if (this.elements.urlInput)
            this.elements.urlInput.value = data.bookUrl;
        if (this.elements.targetInput)
            this.elements.targetInput.value = data.targetReviews.toString();
        if (this.elements.stretchInput)
            this.elements.stretchInput.value = data.stretchReviews.toString();
    }
    /**
     * レンダリング
     */
    ...
    async handleAutoFetch(event) {
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                this.emitEvent('book:fetched', { data: result.data, metadata: result.metadata });
            }
            else {
    ...
    displayValidationErrors(result) {
        // エラー表示をリセット
        this.selectAll('.error').forEach(error => this.hide(error));
        ...
        Object.entries(result.errors).forEach(([field, message]) => {
            ...
            const inputElement = this.select(`#${field}`);
            if (errorElement) {
     ...
     * モデルデータを直接保存（排他制御付き）
     */
    async saveModelDataDirectly(operationName) {
...
//# sourceMappingURL=BookInfoForm.js.map