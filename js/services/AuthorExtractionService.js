 ...
 */
import { AuthorExtractionMethod } from '../types/index.js';
import { logger } from '../utils/AILogger.js';
export class AuthorExtractionService {
    constructor(debugMode = false) {
        Object.defineProperty(this, "DEBUG_MODE", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
     ...
     * Tier 1: 構造化データからの抽出
     */
    async extractFromStructuredData(html, debug) {
        ...
        for (const [index, pattern] of jsonLdPatterns.entries()) {
            ...
            const patternInfo = { pattern: pattern.toString(), matches: [] };
     ...
     */
    async extractFromSemanticHTML(html, debug) {
     ...
     * Tier 3: テキストパターンマッチング
     */
    async extractFromTextPatterns(html, debug) {
     ...
     * Tier 4: DOM構造解析
     */
    async extractFromDOMAnalysis(html, debug) {
        if (this.DEBUG_MODE)
     ...
     * 著者名のスコアリング（優先度付け）
     */
    scoreAuthorName(name) {
        let score = 0;
        // 日本語名を優先
        if (/[あ-んア-ンー\u4e00-\u9faf]/.test(name))
            score += 10;
        // 適切な長さ
        ...
        if (/^[A-Z][a-z]/.test(name))
            score += 3;
        // スペースを含む（フルネーム）
        if (/\s/.test(name) && name.split(' ').length <= 4)
            score += 2;
        return score;
    }
    /**
     * 抽出失敗時のログ出力
     */
    async logExtractionFailure(html, url, debug) {
        ...
        debug.htmlSections = relevantSections.map(section => section ? section.substring(0, 200) + '...' : '');
...
//# sourceMappingURL=AuthorExtractionService.js.map