...
import { DEFAULT_MILESTONES, MILESTONE_ICONS, PROGRESS_CONFIG, ANIMATIONS } from '../utils/constants.js';
...
export class ProgressViewer extends BaseComponent {
    constructor(container, context, options = {}) {
        ...
        Object.defineProperty(this, "progressData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "refreshTimerId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    `;
        // DOM要素への参照を取得
        this.elements = {
            container: this.select('.progress-viewer'),
            bookCover: this.select('#bookCover'),
            bookTitle: this.select('#bookTitle'),
            bookAuthor: this.select('#bookAuthor'),
            progressBar: this.select('.progress-bar'),
            progressFill: this.select('.progress-fill'),
            progressText: this.select('#progressText'),
            currentReviews: this.select('#currentReviews'),
            targetReviews: this.select('#targetReviews'),
            stretchReviews: this.select('#stretchReviews'),
            milestoneList: this.select('#milestoneList'),
            refreshButton: this.select('#refreshBtn'),
            backButton: this.select('#backBtn'),
            lastUpdated: this.select('#lastUpdated'),
            shareButton: this.select('#shareBtn') ?? undefined,
        };
    }
    /**
     * 書籍情報セクション
     */
    createBookInfoSection() {
        return `
     ...
     * プログレスバーをアニメーション
     */
    async animateProgressBar(targetWidth) {
        if (!this.elements.progressFill)
            return;
     ...
     * プログレスバーの色を更新
     */
    updateProgressBarColor(achievementRate) {
        if (!this.elements.progressFill)
            return;
     ...
     */
    renderMilestones() {
        if (!this.elements.milestoneList || !this.progressData)
            return;
     ...
     * 更新メッセージを表示
     */
    showUpdateMessage(type, message) {
        // 一時的なメッセージ要素を作成
...
//# sourceMappingURL=ProgressViewer.js.map