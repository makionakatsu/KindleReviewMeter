...
import { ApplicationContext } from './types/index.js';
...
class VisualDisplayApp {
  ...
  private progressViewer: ProgressViewer | null = null;
  ...
  private createApplicationContext(): ApplicationContext {
    ...
    const storage = {
      ...
      exists: (key: string) => localStorage.getItem(key) !== null,
  ...
  private enableDebugMode(): void {
    ...
    (window as any).visualAppContext = this.context;
    (window as any).visualApp = this;
  ...
  private setupShareFunctionality(): void {
    if (!this.shareService || !this.progressViewer) return;
    ...
    this.context.eventEmitter.on('share:started', async (event) => {
      try {
        ...
        // シェア画像を生成
        const result = await this.shareService!.generateShareImage(
          data,
          progressData,
          options
        );
        ...
        alert('シェア機能でエラーが発生しました: ' + (error as Error).message);
      }
    });
  }
   ...
   * シェアオプションを表示
   */
  private showShareOptions(shareResult: any): void {
    // モーダルダイアログを作成
   ...
   * シェアモーダルのイベントを設定
   */
  private setupShareModalEvents(modal: HTMLElement, shareResult: any): void {
    const closeModal = () => {
    ...
    modal.querySelector('.share-download')?.addEventListener('click', () => {
      this.shareService!.downloadImage(
        shareResult.imageUrl,
        `kindle-review-progress-${Date.now()}.png`
      );
    });
  ...
  public async destroy(): Promise<void> {
    ...
    try {
      ...
      // イベントエミッターをクリーンアップ
      (this.context.eventEmitter as any).removeAllListeners();
