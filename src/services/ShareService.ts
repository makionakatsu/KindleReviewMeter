...
import { BookData, ProgressData, ShareOptions, ShareResult } from '../types/index.js';
import { 
  SHARE_CONFIG, 
  COLORS, 
  FONTS, 
...
export interface ShareServiceOptions {
  defaultShareOptions?: Partial<ShareOptions>;
  enableWatermark?: boolean;
  enableTwitterShare?: boolean;
  customTemplate?: ShareTemplate;
}
...
export class ShareService {
   ...
   * Canvasを設定
   */
  private setupCanvas(options: ShareOptions): void {
   ...
   * 装飾要素を描画
   */
  private drawDecorationElements(): void {
    const template = this.options.customTemplate;
   ...
   * 書籍情報を描画
   */
  private async drawBookInfo(bookData: BookData): Promise<void> {
    const template = this.options.customTemplate;
    const margin = 40;
   ...
   * 書籍アイコンを描画
   */
  private drawBookIcon(y: number): void {
   ...
   * プログレス情報を描画
   */
  private drawProgress(progressData: ProgressData): void {
    const template = this.options.customTemplate;
    const margin = 40;
    ...
    this.ctx.fillText(
      ...
      margin + progressWidth / 2, 
      progressY + barHeight / 2 + 6
    );
   ...
   * 統計情報を描画
   */
  private drawStatistics(progressData: ProgressData, startY: number): void {
    const template = this.options.customTemplate;
    const stats = [
   ...
   * マイルストーンを描画
   */
  private drawMilestones(bookData: BookData, progressData: ProgressData): void {
    const template = this.options.customTemplate;
   ...
   * プログレス率に応じた色を取得
   */
  private getProgressColor(achievementRate: number): string {
    if (achievementRate >= 100) return COLORS.SUCCESS;
    if (achievementRate >= 75) return COLORS.INFO;
    if (achievementRate >= 50) return COLORS.WARNING;
    return COLORS.ERROR;
  }
   ...
   * 画像データを生成
   */
  private generateImageData(options: ShareOptions): string {
   ...
   * CanvasをBlobに変換
   */
  private canvasToBlob(options: ShareOptions): Promise<Blob | null> {
   ...
   * シェア用URLを生成
   */
  private generateShareUrl(bookData: BookData, progressData: ProgressData): string {
    const text = encodeURIComponent(
      `📚「${bookData.bookTitle}」のレビュー数進捗\n` +
   ...
   * ソーシャルメディアにシェア
   */
  public shareToSocial(shareUrl: string, platform: 'twitter' | 'facebook' = 'twitter'): void {
   ...
   * 画像をダウンロード
   */
  public downloadImage(imageUrl: string, filename: string = 'progress-share.png'): void {
   ...
   * カスタムテンプレートを設定
   */
  public setTemplate(template: Partial<ShareTemplate>): void {
   ...
   * デフォルトシェアオプションを更新
   */
  public updateDefaultOptions(options: Partial<ShareOptions>): void {
